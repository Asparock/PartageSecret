...
public class Protocole1_S extends javax.swing.JFrame {

    private SimpleManager simpleMan = new SimpleManager();
    private StringManager stringMan = new StringManager();
   
    public Protocole1_S() {
        initComponents();
        this.setTitle("Procole 1 - Secret");
    ...
    private void rebuildActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rebuildActionPerformed
        if(!(this.key_2.getText().equals("")) && !(this.key_1.getText().equals("")) && this.key_1.getText().matches("([0-1])+") && this.key_2.getText().matches("([0-1])+")){
            this.key.setText(simpleMan.getBinFromKeys(this.key_2.getText(), this.key_1.getText()));
        } else {
